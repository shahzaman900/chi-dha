...
export function PhrDisplay({ patientId }: { patientId: string }) {
  ...
  const [activeModal, setActiveModal] = useState<{
    isOpen: boolean;
    view: "assessment" | "treatment" | "default";
  }>({ isOpen: false, view: "default" });
  ...
  // Side sheets & event detail state
  ...
  // Use live timeline from store (updates in real-time)
  ...
  return (
    <div className="flex overflow-auto flex-col bg-card text-foreground h-full">
      {/* Enhanced Header with Demographics + Actions */}
      <div className="flex items-center justify-between px-6 py-3 bg-white border-b border-border">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
              <span>{patient.profile.age}y</span>
              {storePatient?.gender && (
                <>
                  <span className="text-slate-300">•</span>
                  <span>{storePatient.gender}</span>
                </>
              )}
              {storePatient?.phone && (
                <>
                  ...
                  <span>{storePatient.phone}</span>
                </>
              )}
              {storePatient?.mrn && (
                <>
                  <span className="text-slate-300">•</span>
                  <span className="font-mono">{storePatient.mrn}</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex items-center justify-center gap-2 flex-1 px-6">
          <Button
          ...
          <Button
            className="bg-indigo-600 hover:bg-indigo-700 text-white h-8 px-3 rounded-full text-[11px] font-semibold tracking-wide shadow-sm transition-all border-none"
            onClick={() => setActiveSheet("escalate")}
          >
            <Stethoscope className="h-3.5 w-3.5 mr-1.5" /> Refer to Doctor
          </Button>
          <Button
            className="bg-red-600 hover:bg-red-700 text-white h-8 px-3 rounded-full text-[11px] font-semibold tracking-wide shadow-sm transition-all border-none"
            onClick={() => setActiveSheet("emergency")}
          >
            <Siren className="h-3.5 w-3.5 mr-1.5" /> Emergency
          </Button>
          <Button
            className="bg-emerald-600 hover:bg-emerald-700 text-white h-8 px-3 rounded-full text-[11px] font-semibold tracking-wide shadow-sm transition-all border-none"
            onClick={() => setActiveSheet("resolve")}
          >
            <CheckCircle2 className="h-3.5 w-3.5 mr-1.5" /> Resolve
          </Button>
          <Button
            variant="outline"
            className="h-8 px-3 rounded-full text-[11px] font-semibold tracking-wide border-slate-300 text-slate-600 hover:bg-slate-50"
            onClick={() => openPhrTab(patientId, patient.profile.name, "phr")}
          >
            <FileText className="h-3.5 w-3.5 mr-1.5" /> Go to PHR
          </Button>
        </div>
        <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
          <div className="text-center bg-slate-200/60 px-4 py-1.5 rounded-xl border border-slate-200 flex flex-col items-center justify-center min-w-[80px]">
          ...
          <Badge
            variant="outline"
            className={`text-xs font-bold tracking-wide px-3 py-1.5 rounded-full flex items-center border ${
              isCritical
          ...
          >
            ...
            {storePatient?.status || patient.ews.status}
          </Badge>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left Column - Patient Info & Metrics & Sparklines */}
        <div className="lg:col-span-1 space-y-6">
          {/* Patient Profile Card */}
          <Card className={`bg-card shadow-sm border ${borderColor}`}>
            <CardHeader className="pb-2">
              <CardTitle className={`text-lg ${textAccent}`}>
                Patient Profile
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex justify-between items-start">
                ...
                <span className="text-foreground font-medium text-right max-w-[180px]">
                  {patient.profile.conditions.join(", ")}
                </span>
              </div>
              <Separator className="bg-secondary" />
              <div className="flex justify-between">
            ...
            <CardContent className="grid grid-cols-2 gap-4">
              {Object.entries(patient.metrics).map(
                ([key, data]: [string, any]) => (
                  <div
                    key={key}
                    className={`p-3 rounded-lg bg-muted border ${borderColor}`}
                  >
                    ...
                    <div
                      className={`text-xl font-bold ${textAccent === "text-orange-400" ? "text-orange-600" : "text-red-500"}`}
                    >
                      {data.value}
                    </div>
                    {data.change && (
          ...
          >
            <CardHeader className="pb-2 shrink-0">
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle
                    className={`text-lg flex items-center gap-2 ${patient.profile.name === "Patient 3" ? "text-orange-500" : "text-orange-600"}`}
                  >
                    <Activity className="h-5 w-5" />
                    {patient.diagnosisMetadata?.title ||
                ...
                {timeline.map((event, i) => (
                  <div key={i} className="relative">
                    ...
                    <div className="flex flex-col gap-1">
                      <div className="text-brand-500 font-bold text-xs">
                        {event.time}
                      </div>
                      <div
                        className={`text-sm leading-snug ${event.type === "critical" || event.type === "critical-action" ? "text-red-400 font-bold" : "text-muted-foreground"}`}
                      >
                        {event.event}
                      </div>

                      {/* View Detailed button for events with detailType */}
                      {event.detailType && event.details && (
                        <div className="mt-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 px-3 text-[11px] font-semibold border border-brand-500/30 text-brand-600 hover:bg-brand-50 hover:text-brand-700 bg-white/50 rounded-md"
                      ...
                      {/* Legacy view details for PHR-sourced events */}
                      {!event.detailType &&
                        (event.event.includes("Symptom Checker") ||
                          event.event.includes("AI Assessment") ||
                          event.event.includes("Nurse Sarah") ||
                          event.event.includes(
                            "EMERGENCY services dispatched",
                          ) ||
                          event.event.includes("Nurse Escalation")) && (
                          <div className="mt-2">
                            <Button
                              variant="ghost"
                              size="sm"
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          setActiveModal((prev) => ({ ...prev, isOpen: open }))
        }
      >
              ...
              {patient.diagnosisDetails.map((detail, idx) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="text-lg font-bold text-yellow-600 tracking-wide">
                      {detail.name}
                    </h3>
                    <div className="flex items-center gap-4">
                      <div className="flex flex-col items-end">
                        <span className="text-2xl font-bold text-yellow-600 leading-none">
                          {detail.probability}%
                        </span>
                        <span className="text-[10px] text-yellow-500/50 uppercase font-bold tracking-wider">
                          Probability
                        </span>
                      </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-muted grid grid-cols-1 md:grid-cols-2 gap-6 custom-scrollbar">
                <div className="border border-green-500/30 rounded-lg overflow-hidden flex flex-col">
                  ...
                  <div className="p-6 space-y-6 flex-1 bg-card/50 max-h-[300px] overflow-y-auto custom-scrollbar">
                    {patient.treatmentDetails.immediateInterventions.sections.map(
                      (section, idx) => (
                        <div key={idx}>
                          <h4 className="text-brand-600 font-bold mb-2 text-sm">
                            {section.name}
                          </h4>
                ...
                <div className="border border-brand-500/30 rounded-lg overflow-hidden flex flex-col">
                  ...
                  <div className="p-6 space-y-6 flex-1 bg-card/50 max-h-[300px] overflow-y-auto custom-scrollbar">
                    {patient.treatmentDetails.monitoring.sections.map(
                      (section, idx) => (
                        <div key={idx}>
                          <h4
                            className={`font-bold mb-2 text-sm ${section.name.includes("Red Flags") ? "text-red-400" : "text-brand-600"}`}
                          >
                            {section.name}
                          </h4>
                          <ul className="space-y-2">
                            {section.items.map((item, i) => (
                              ...
                              >
                                <div
                                  className={`h-1.5 w-1.5 rounded-full mt-1.5 shrink-0 ${section.name.includes("Red Flags") ? "bg-red-500" : "bg-slate-500"}`}
                                ></div>
                                {item}
                              </li>
                            ))}
                  ...
                  <div className="border border-violet-500/30 rounded-lg overflow-hidden flex flex-col">
                    ...
                    <div className="p-6 space-y-6 flex-1 bg-card/50 max-h-[300px] overflow-y-auto custom-scrollbar">
                      <ul className="space-y-4">
                        {patient.treatmentDetails.nurseDispatch.items.map(
                ...
                {patient.treatmentDetails.labDispatch && (
                  <div className="border border-amber-500/30 rounded-lg overflow-hidden flex flex-col">
                    ...
                    <div className="p-6 space-y-6 flex-1 bg-card/50 max-h-[300px] overflow-y-auto custom-scrollbar">
                      <ul className="space-y-4">
                        {patient.treatmentDetails.labDispatch.items.map(
                          (item: string, i: number) => (
                            <li
                              key={i}
                              className="flex items-start gap-3 text-muted-foreground text-sm"
      ...
      {/* Side Sheets for encounter actions */}
      <EmergencySheet
      ...
      <EscalateSheet
        patient={storePatient}
        open={activeSheet === "escalate"}
        onClose={() => setActiveSheet(null)}
        onConfirm={escalateToDoctor}
      />
      <ResolveSheet
        patient={storePatient}
        open={activeSheet === "resolve"}
        onClose={() => setActiveSheet(null)}
