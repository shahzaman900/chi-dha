...
export function EncountersTable() {
  const { patients, selectedPatientId, setSelectedPatientId, openPhrTab } =
    usePatientStore();

  // Context Menu State
  const [contextMenuOpenId, setContextMenuOpenId] = useState<string | null>(
    null,
  ); // This will now hold an encounter ID, not just patient ID
  ...
  const handleRowClick = (e: React.MouseEvent, encounterId: string) => {
    ...
    setContextMenuOpenId(
      contextMenuOpenId === encounterId ? null : encounterId,
    );
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={() =>
                        setSelectedPatientId(
                          patient.id === selectedPatientId ? null : patient.id,
                        )
                      }
                      onClick={(e: React.MouseEvent) =>
                        handleCheckboxClick(e, patient.id)
                      }
          ...
          >
            <div className="w-[180px] p-1 border border-slate-200 shadow-xl rounded-lg bg-white overflow-hidden">
              <div
                ...
                onClick={() => {
                  const match = allEncounters.find(
                    (e) => e.encounter.id === contextMenuOpenId,
                  );
                  if (match)
                    handleViewDetails(match.patient.id, match.patient.name);
                }}
              >
      ...
      <div className="flex items-center justify-between px-6 py-3 bg-white border-t border-slate-200 text-sm text-slate-500 w-full shrink-0">
        ...
        <div className="flex items-center gap-6 text-sm">
          ...
          <span className="font-medium text-slate-700">
            1 - {allEncounters.length} of {allEncounters.length}
          </span>
