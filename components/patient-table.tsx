...
export function PatientTable() {
  ...
  // Action Sheet State (patientId controls open/close; form state lives in extracted components)
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-white w-full h-full relative" onClick={() => setContextMenuOpenId(null)}>
        ...
        <Table>
          <TableHeader className="bg-[#eaf3fd] sticky top-0 z-10 shadow-sm shadow-[#eaf3fd]/50">
            <TableRow className="border-none hover:bg-transparent">
              ...
              <TableHead className="text-slate-800 font-bold text-[13px] whitespace-nowrap">Vitals Trend</TableHead>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex flex-col items-center">
                          <SparkLine 
                            ...
                            fillColor="#ef4444" 
                          />
                          <span className="text-[9px] text-slate-400 font-medium mt-0.5">HR</span>
                        </div>
                        <div className="flex flex-col items-center">
                          <SparkLine 
                            ...
                            fillColor="#3b82f6" 
                          />
                          <span className="text-[9px] text-slate-400 font-medium mt-0.5">SpO₂</span>
                        </div>
                      </div>
                    ) : (
      ...
      {/* Action Sheets (Extracted Components) */}
      <AcknowledgeSheet
      ...
      />
      <EmergencySheet
        patient={patients.find(p => p.id === emergencySheetPatientId)}
        open={!!emergencySheetPatientId}
        onClose={() => setEmergencySheetPatientId(null)}
        onConfirm={triggerEmergency}
      />
      <EscalateSheet
        patient={patients.find(p => p.id === escalateSheetPatientId)}
        open={!!escalateSheetPatientId}
        onClose={() => setEscalateSheetPatientId(null)}
        onConfirm={escalateToDoctor}
      />
      <ResolveSheet
        patient={patients.find(p => p.id === resolveSheetPatientId)}
        open={!!resolveSheetPatientId}
        onClose={() => setResolveSheetPatientId(null)}
